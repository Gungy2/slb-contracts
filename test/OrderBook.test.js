...
describe("OrderBook", () => {
  ...
  describe("#place buy order not matching", () => {
    ...
    it("should not buy with price 0", async () => {
      await expect(
        book.connect(acc1).placeBuyOrder(0, parseAmount(1000, 18))
      ).eventually.rejectedWith("Can not place order with price equal 0");
    });

    it("should place first buy", async () => {
      await book.connect(acc1).placeBuyOrder(1, parseAmount(1000, 18));

      const [buyOrdersInStepCounter, step] = await Promise.all([
    ...
    it("should place second buy", async () => {
      ...
      expect(order1InStep2.amount.toString()).to.eq(
        parseAmount(1000, 18).toString()
      );
      expect(order1InStep2.maker).to.eq(await acc2.getAddress());

      expect(order2InStep2.amount.toString()).to.eq(
        parseAmount(500, 18).toString()
      );
  ...
  describe("#place sell order not matching", () => {
    it("should not sell if not enough money", async () => {
      await expect(
        book.connect(acc1).placeSellOrder(0, parseAmount(1001, 18))
      ).eventually.rejectedWith(
        "VM Exception while processing transaction: reverted with reason string 'ERC20: insufficient allowance'"
      );
    });
    ...
    it("should not buy with price 0", async () => {
      await expect(
        book.connect(acc1).placeSellOrder(0, parseAmount(1000, 18))
      ).eventually.rejectedWith("Can not place order with price equal 0");
    });
    ...
    it("should place first sell", async () => {
      ...
      const [sellOrdersInStepCounter, step] = await Promise.all([
  ...
  describe("#place buy order match with current sell orders ", () => {
    it("should place an instant buy (perfect match)", async () => {
      ...
      expect(sellStep2.amount.toString()).to.eq(
        parseAmount(500, 18).toString()
      );
    ...
    it("should place an over step buy", async () => {
      ...
      expect(buyOrderInStep1.amount.toString()).to.eq(
        parseAmount(500, 18).toString()
      );
    ...
    it("should place an over 2 step buy", async () => {
      ...
      expect(sellOrdersInStep51.amount.toString()).to.eq(
        parseAmount(50, 18).toString()
      );
  ...
  describe("#place sell order match with current buy orders ", () => {
    ...
    it("should place an over step buy", async () => {
      ...
      expect(sellOrderInStep2.amount.toString()).to.eq(
        parseAmount(500, 18).toString()
      );
    ...
    it("should place an over 2 step buy", async () => {
      ...
      expect(sellStep4.amount.toString()).to.eq(
        parseAmount(100, 18).toString()
      );
      expect(sellStep4.lowerPrice.toNumber()).to.eq(0);
      expect(sellStep4.higherPrice.toNumber()).to.eq(5);

      expect(sellStep5.amount.toString()).to.eq(
        parseAmount(100, 18).toString()
      );
      expect(sellStep5.lowerPrice.toNumber()).to.eq(4);
      expect(sellStep5.higherPrice.toNumber()).to.eq(0);

      expect(buyOrdersInStep51.amount.toString()).to.eq(
        parseAmount(50, 18).toString()
      );
